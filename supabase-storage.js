 ...
 *  supabase-storage.js  |  Add to GitHub alongside index.html
 * ════════════════════════════════════════════════════════
 *
 *  HOW IT WORKS:
 *  • Saves to localStorage immediately (keeps the UI fast)
 *  • Syncs to Supabase in the background automatically
 *  • On page load: shows local data first, then refreshes from cloud
 *  • All teammates see the same live data
 * ════════════════════════════════════════════════════════
 */
...
// ════════════════════════════════════════════════════════
//  2.  UI — Config Modal + Button (injected dynamically)
// ════════════════════════════════════════════════════════
(function _injectUI() {
  // Config Modal
    ...
    <div class="modal" style="width:500px;max-width:95vw;">
      ...
      <div class="modal-body">
        <p style="font-size:12px;color:#7A7A8A;margin-bottom:14px;line-height:1.6;">
          ...
          <strong>supabase.com → Project → Settings → API Keys</strong><br>
          Your Project URL: <strong>https://pfknkwjoxixgzdzznegr.supabase.co</strong>
        </p>
        <div style="margin-bottom:11px;">
          <label style="display:block;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px;">Project URL</label>
          <input id="sb-url" type="text" placeholder="https://xxxxxxxxxxxx.supabase.co"
            style="width:100%;font-size:13px;padding:9px 12px;border-radius:7px;border:1.5px solid #E0C8D8;outline:none;font-family:Inter,sans-serif;">
        </div>
        <div style="margin-bottom:11px;">
          <label style="display:block;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px;">Publishable / Anon Key</label>
          <input id="sb-key" type="text" placeholder="sb_publishable_... or eyJhbGci..."
            style="width:100%;font-size:13px;padding:9px 12px;border-radius:7px;border:1.5px solid #E0C8D8;outline:none;font-family:Inter,sans-serif;">
        </div>
        <div style="background:#FFF5FC;border:1px solid #F0C0E0;border-radius:6px;padding:9px 12px;font-size:11px;color:#7A7A8A;line-height:1.6;">
          ⚠ Run the SQL from <strong>SUPABASE_SETUP.md</strong> in Supabase SQL Editor first.
        </div>
        <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:16px;">
          <button id="sb-cancel" style="background:transparent;border:1.5px solid #EADDF0;color:#7A7A8A;border-radius:7px;padding:8px 16px;font-size:13px;cursor:pointer;">Cancel</button>
          <button id="sb-save" style="background:linear-gradient(135deg,#E020A0,#C8189A);color:#fff;border:none;border-radius:7px;padding:8px 20px;font-size:14px;font-weight:700;cursor:pointer;font-family:'Rajdhani',sans-serif;">☁ Save &amp; Connect</button>
        </div>
      </div>
    </div>`;
  document.body.appendChild(modal);

  // Cloud Config Button in header-subtitle bar
  const subtitle = document.querySelector('.header-subtitle');
  if (subtitle) {
    const btn = document.createElement('button');
    btn.id = 'sb-config-btn';
    btn.innerHTML = '☁ Cloud Config';
    btn.style.cssText = `margin-left:auto;background:transparent;border:1.5px solid rgba(255,255,255,0.25);
      color:rgba(255,255,255,0.75);border-radius:5px;padding:3px 11px;font-size:11px;font-weight:600;
      cursor:pointer;font-family:Inter,sans-serif;`;
    btn.onmouseover = () => { btn.style.borderColor='#F040B8'; btn.style.color='#fff'; };
    btn.onmouseout  = () => { btn.style.borderColor='rgba(255,255,255,0.25)'; btn.style.color='rgba(255,255,255,0.75)'; };
...
// ════════════════════════════════════════════════════════
//  3.  OVERRIDE saveData — localStorage + Supabase
// ════════════════════════════════════════════════════════
function saveData() {
  ...
  // Supabase – async background
  if (_sb) _syncToSupabase().catch(e => console.warn('[Supabase] sync error:', e));
}
...
async function _syncToSupabase() {
  ...
  // Lists
  await _sb.from('ail_lists').upsert([
  ...
  // Wiring Assignments
  const waRows = Object.entries(wiringAssign).map(([no, a]) => ({
    project_no: no, person: a.person||'', start_date: a.startDate||'',
    end_date: a.endDate||'', note: a.note||'', updated_at: now,
  }));
  if (waRows.length) {
    const { error } = await _sb.from('ail_wiring_assign').upsert(waRows, { onConflict: 'project_no' });
    if (error) console.warn('[Supabase] wiring_assign error:', error.message);
  }
...
async function loadFromSupabase() {
  ...
  try {
    ...
    // Projects
    const { data: projs } = await _sb.from('ail_projects').select('*').order('created_at',{ascending:true});
    if (projs && projs.length) {
      projects = projs.map(p => ({
        no: p.no, customer: p.customer||'', type: p.type||'',
        priority: p.priority||'Medium', salesperson: p.salesperson||'',
        engineering: p.engineering||_blank(), purchase: p.purchase||_blank(),
        fabrication: p.fabrication||_blank(), wiring: p.wiring||_blank(),
        testing: p.testing||_blank(), dispatch: p.dispatch||_blank(),
      }));
    }

    // Changelog
    const { data: cl } = await _sb.from('ail_changelog').select('*').order('created_at',{ascending:false});
    if (cl && cl.length) {
      changelog = cl.map(c => ({
        time:c.time, project:c.project, customer:c.customer, salesperson:c.salesperson,
        stage:c.stage, resp:c.resp, change:c.change, date:c.date, remark:c.remark,
      }));
      _clSyncedCount = changelog.length;
    }

    // Wiring Assignments
    const { data: wa } = await _sb.from('ail_wiring_assign').select('*');
    if (wa && wa.length) {
      wiringAssign = {};
      wa.forEach(a => {
        wiringAssign[a.project_no] = {
          person: a.person||'', startDate: a.start_date||'',
          endDate: a.end_date||'', note: a.note||'',
        };
      });
    }
...
// ════════════════════════════════════════════════════════
//  6.  PATCH deleteProject — also deletes from Supabase
// ════════════════════════════════════════════════════════
const _origDeleteProject = typeof deleteProject === 'function' ? deleteProject : null;
